...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Regex expression (start of string, 0-9A-Z one or more to end of string), spaces one or more, with and or or criteria, additional spaces, and without regard to case (icase)
  std::regex SQLInjectionCriteria("[^0-9A-Z+$]\\s*(and|or)\\s*[\\'\"]?", std::regex_constants::icase);
    
  // Using the regex built, compare with the sql and ensure that it follows proper format, otherwise throw an error or attempt to inject
  if (std::regex_search(sql, SQLInjectionCriteria)) {
      std::cout << "SQL Injection detected: The following error is associated -> " << &error_message << std::endl;
      return false;
  }
