...
std::string read_file(const std::string& filename)
{
  ...
  std::ifstream input_text_file(filename);
  std::stringstream buffer;
  buffer << input_text_file.rdbuf();
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    struct tm ltm;
    localtime_s(&ltm, &now);
    int year_time = 1900 + ltm.tm_year;
    int month_time = 1 + ltm.tm_mon;
    int day_time = ltm.tm_mday;

    std::ofstream write_file(filename);

    write_file << student_name << std::endl;
    write_file << year_time << "-" << month_time << "-" << day_time << std::endl;
    write_file << key << std::endl;
